...
class DownloadAndUnpackApp(QtWidgets.QWidget):
    def __init__(self):
        ...
        # Run the permission test during initialization
        self.check_permissions()

    def setup_ui(self):
        self.setWindowTitle("Resident Evil AIO DownloadUnpackGUI PyQt5 Version 2_Linux")
    ...
    def download_image(self, url, folder_name):
        ...
        background_dir = os.path.join(documents_dir, folder_name)
    ...
    def show_help(self):
        help_text = (
            ...
            ".zips can be stored in root directory but they don't get deleted when stored there.\n\n"
            ...
            "You just use an archive manager and dump the game into the correct named folder from the ISO.\n\n"
    ...
    def get_desktop_path(self):
        ...
        if os.path.exists(desktop_path):
            self.log_message("Using Desktop path.")
            return desktop_path
        else:
            self.log_message("Desktop path not found.", error=True)
            return user_home

    def check_permissions(self):
        """Check if the script has write and delete permissions in the Documents directory."""
        test_file = os.path.join(os.path.expanduser("~/Documents"), "permission_test.txt")
        try:
            with open(test_file, 'w') as f:
                f.write("Testing write permissions.")
            os.remove(test_file)
            self.log_message("Permission check passed: Able to write and delete files.")
        except Exception as e:
            self.log_message("Permission check failed: " + str(e), error=True)
