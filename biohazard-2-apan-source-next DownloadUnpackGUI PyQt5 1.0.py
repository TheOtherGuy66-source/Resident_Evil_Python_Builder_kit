...
class DownloadAndUnpackApp(QtWidgets.QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("biohazard-2-apan-source-next DownloadUnpackGUI PyQt5 1.0")
        ...
        background_dir = os.path.join(appdata_dir, 'BIO2PyQt5Background')
        ...
        source_next_menu = menubar.addMenu("biohazard-2-apan-source-next")
        source_next_menu.addAction("biohazard-2-apan-source-next", lambda: run_async(self.biohazard_sourcenext()))
        source_next_menu.addAction("Download Bio2_mod.zip", self.download_biohazard2)
    ...
    async def biohazard_sourcenext(self):
        ...
        zip_path = self.find_zip_file('Bio2_mod.zip')
        if not zip_path:
            self.log_message("Bio2_mod.zip not found. Please download the file first.", error=True)
            download_confirm = QtWidgets.QMessageBox.question(self, "Download Required", "Did you download Bio2_mod.zip?", QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
        ...
        temp_extract_dir = os.path.join(desktop_dir, 'biohazard-2-apan-source-next', 'temp_extract')
    ...
    def auto_select_destination(self):
        desktop_dir = self.get_desktop_path()
        self.dest_dir = os.path.join(desktop_dir, 'biohazard-2-apan-source-next', 'data')
    ...
    def auto_cleanup_files(self):
        mediakite_dir = os.path.join(self.get_desktop_path(), 'biohazard-2-apan-source-next')
        if os.path.exists(mediakite_dir):
            shutil.rmtree(mediakite_dir)

        # Rename 'data' folder to 'RE2SHDP - RE Seamless HD Project' with context-specific suffix
        data_path = os.path.join(self.get_desktop_path(), 'data')
        suffix = "biohazard-2-apan-source-next" if self.renaming_context == "biohazard-2-apan-source-next" else self.renaming_context
        new_name = os.path.join(self.get_desktop_path(), f'RE2SHDP - RE Seamless HD Project ({suffix})')
        if os.path.exists(data_path):
            if os.path.exists(new_name):
                shutil.rmtree(new_name)
            os.rename(data_path, new_name)
            self.log_message(f"Renamed 'data' folder to 'RE2SHDP - RE Seamless HD Project ({suffix})'")
