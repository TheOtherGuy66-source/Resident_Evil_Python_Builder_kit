...
class DownloadAndUnpackApp(QtWidgets.QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("BIOHAZARD Mediakite DownloadUnpackGUI PyQt5 2.4")
        ...
        background_dir = os.path.join(appdata_dir, 'BIO1PyQt5Background')
        ...
        mediakite_menu = menubar.addMenu("BIOHAZARD Mediakite")
        mediakite_menu.addAction("BIOHAZARD Mediakite", lambda: run_async(self.biohazard_mediakite()))
        mediakite_menu.addAction("Download Biohazard_mod.zip", self.download_biohazard)
        mediakite_menu.addAction("Download dgVoodoo_AMD_fix.zip", self.download_dgvoodoo)
        ...
        self.auto_button = QtWidgets.QPushButton("Auto AMD", self)
        self.auto_button.setStyleSheet("background-color: #333333; color: white;")
        self.auto_button.clicked.connect(lambda: run_async(self.auto_amd_process()))
        ...
        button_layout.addWidget(self.auto_nvidia_button)
    ...
    def copy_to_desktop(self, target_dir):
        desktop_path = os.path.join(self.get_desktop_path(), 'horr')
        if os.path.exists(desktop_path):
            shutil.rmtree(desktop_path)
        shutil.copytree(target_dir, desktop_path, dirs_exist_ok=True)
        self.log_message(f"Copied 'horr' folder to {desktop_path}")
    ...
    def auto_cleanup_files(self):
        mediakite_dir = os.path.join(self.get_desktop_path(), 'BIOHAZARD Mediakite')
        if os.path.exists(mediakite_dir):
            shutil.rmtree(mediakite_dir)

        # Rename 'horr' folder to 'RE1SHDP - RE Seamless HD Project' with context-specific suffix
        horr_path = os.path.join(self.get_desktop_path(), 'horr')
        suffix = "BIOHAZARD Mediakite" if self.renaming_context == "BIOHAZARD Mediakite" else self.renaming_context
        new_name = os.path.join(self.get_desktop_path(), f'RE1SHDP - RE Seamless HD Project ({suffix})')
        if os.path.exists(horr_path):
            if os.path.exists(new_name):
                shutil.rmtree(new_name)
            os.rename(horr_path, new_name)
            self.log_message(f"Renamed 'horr' folder to 'RE1SHDP - RE Seamless HD Project ({suffix})'")
